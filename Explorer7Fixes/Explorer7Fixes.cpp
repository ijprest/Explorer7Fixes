...
class ATL_NO_VTABLE CExplorerBHO :
	...
	public IDispatchImpl<IExplorer7FixesBHO, &IID_IExplorer7FixesBHO, &LIBID_Explorer7FixesLib, /*wMajor =*/ 1, /*wMinor =*/ 0>,
...
STDMETHODIMP CExplorerBHO::OnNavigateComplete( IDispatch *pDisp, VARIANT *URL )
{
	if( m_pBrowser )
	{
        ...
        // until the view has been refreshed.  Rather than call Refresh, we just 
				...
				UINT unViewMode = 0;
				pFolderView->GetCurrentViewMode(&unViewMode);
        if( unViewMode != FVM_DETAILS ) 
				{
					pFolderView->SetCurrentViewMode( unViewMode == FVM_LIST ? FVM_SMALLICON : FVM_LIST );
					pFolderView->SetCurrentViewMode( unViewMode );
				}
			}
		}
	}
